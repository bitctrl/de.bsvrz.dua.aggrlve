...
public class AggregationsFahrStreifen extends AbstraktAggregationsObjekt
	 ...
	 * @param fs
	 *            der Fahrstreifen dessen Aggregationsdaten ermittelt werden
	 *            sollen
	...
	public AggregationsFahrStreifen(final ClientDavInterface dav,
			final FahrStreifen fs) throws DUAInitialisierungsException {
			...
			if (!basisDaten.isEmpty()) {
				...
				for (AggregationsAttribut attribut : AggregationsAttribut
					...
					if (attribut.isGeschwindigkeitsAttribut()
						...
						this.aggregiereMittel(attribut, nutzDatum, basisDaten,
								zeitStempel, intervall);
					} else {
						this.aggregiereAlsBasisDatum(attribut, nutzDatum,
								basisDaten, zeitStempel, intervall);
					}
				}
			}
	 ...
	 * @param basisDaten
	 *            die der Aggregation zu Grunde liegenden Daten (muss mindestens

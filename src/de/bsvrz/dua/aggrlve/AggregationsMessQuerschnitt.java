...
extends AbstraktAggregationsObjekt{
	 ...
	 */
	private Map<SystemObject, AggregationsFahrStreifen> fsMenge = 
			new HashMap<SystemObject, AggregationsFahrStreifen>();
	...
	/**
	 * Erfragt ein Aggregationsobjekt unterhalb dieses Objektes
	 * (nur fuer Testzwecke)
	 * 
		...
		for(AggregationsAttribut attribut:AggregationsAttribut.getInstanzen()){
			...
			if(attribut.isGeschwindigkeitsAttribut()){
				...
				for(AggregationsFahrStreifen fahrStreifen:fsDaten.keySet()){
					Collection<AggregationsDatum> fsQuellDatum = fsDaten.get(fahrStreifen);
				...
				for(Collection<AggregationsDatum> fsQuellDatum:fsDaten.values()){
					...
					if(erfassungsIntervall > 0){
						summe += zwischenSumme * (double)Constants.MILLIS_PER_HOUR / erfassungsIntervall;
					}
				}

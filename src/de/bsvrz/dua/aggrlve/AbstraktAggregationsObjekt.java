...
public abstract class AbstraktAggregationsObjekt {
	 ...
	 * die restlichen auszufuellenden Attribute der Attributgruppen
	 * <code>atg.verkehrsDatenKurzZeitFs</code> bzw <code>atg.verkehrsDatenKurzZeitMq</code>,
	 * die innerhalb der DTV-Berechnung nicht erfasst werden
	 */
	 ...
	 * @param obj das Systemobjekt, das hier verwaltet wird
	 */
	 ...
	 * aggregiert werden soll (gilt eigentlich nur fuer Tagesdaten, da die Anzahl der Stunden
	 * pro Tag zwischen 23 und 25 schwankt) 
			...
			if(zielDatenSoll - zielDaten.size() > 0){
				for(int i = 0; i<zielDatenSoll - zielDaten.size(); i++){
					if(wertAnzahl > 0){
						...
						zielDaten.add(new AggregationsAttributWert(attribut, DUAKonstanten.NICHT_ERMITTELBAR, 0));
					}
				}
			}
		}else{
	...
	/**
	 * Erfragt, ob fuer dieses Objekt ein Datum des uebergebenen Aggregationsintervalls
	 * fuer den uebergebenen Zeitpunkt berechnet werden muss<br>
	 * <b>Achtung:</b> Fuer die Intervalle 1 - 60Minuten ist eine Berechnung immer notwendig.
	 * Fuer alle anderen Intervalle ist eine Berechung nur dann notwendig, wenn nicht schon
	 * ein entsprechendes Datum im Archiv steht (bzw. berechnet wurde)
	 * 
	 * @param zeitStempel ein Zeitpunkt
	 * @param intervall der Aggregationsintervall
	 * @return  ob fuer dieses Objekt ein Datum des uebergebenen Aggregationsintervalls
	 * fuer den uebergebenen Zeitpunkt berechnet werden muss
	 */
	 ...
	 * 
	 * @return ob es sich bei dem hier verwalteten Objekt um eine Objekt vom
	 * Typ <code>typ.fahrStreifen</code> handelt
	 */

...
/**
 * Abstrakte Blaupause fuer einen Ringpuffer, der alle Daten eines bestimmten Aggregationsintervalls
...
public abstract class AbstraktAggregationsPuffer {
	...
	/**
	 * das Aggregationsintervall, fuer das Daten in diesem Puffer stehen ( <code>null</code> deutet
	 * auf messwertersetzte Fahstreifenwerte hin).
	 */
	 ...
	 */
	public AbstraktAggregationsPuffer(final ClientDavInterface dav, final SystemObject obj,
	...
	/**
	 * Aktualisiert diesen Puffer mit neuen Daten. Alte Daten werden dabei aus dem Puffer gelöscht
	 *
	 * @param resultat
	...
	public void aktualisiere(final Dataset resultat) {
		...
		synchronized (ringPuffer) {
			ringPuffer.addFirst(neuesDatum);
			while (ringPuffer.size() > getMaxPufferInhalt()) {
	...
	/**
	 * Erfragt alle in diesem Puffer gespeicherten Datensaetze deren Zeitstempel im Intervall
	 * [begin, ende[ liegen.
	 *
	 * @param begin
	...
	public final Collection<AggregationsDatum> getDatenFuerZeitraum(final long begin,
		...
		synchronized (ringPuffer) {
			for (final AggregationsDatum einzelDatum : ringPuffer) {
				if ((einzelDatum.getDatenZeit() >= begin) && (einzelDatum.getDatenZeit() < ende)) {
	...
	public String toString() {
		String s = "Datenart: "
				+ (aggregationsIntervall == null ? "FS-MWE" : aggregationsIntervall);

		synchronized (ringPuffer) {
			s += "\nMAX: " + getMaxPufferInhalt() + "\nInhalt: "
					+ (ringPufferisEmpty() ? "leer\n" : "\n");
			for (final AggregationsDatum datum : ringPuffer) {
				s += datum + "\n";
			}
		}

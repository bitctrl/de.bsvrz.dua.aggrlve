...
public abstract class AbstraktAggregationsObjekt {
	 ...
	 *            das Systemobjekt, das hier verwaltet wird
	 */
	public AbstraktAggregationsObjekt(final ClientDavInterface dav, final SystemObject obj) {
		this.dav = dav;
		objekt = obj;
		sender = new DAVSendeAnmeldungsVerwaltung(dav, SenderRole.source());
	}
	...
	protected final void sende(final ResultData resultat) {
		if (resultat.getData() == null) {
			final ResultData letztesDatum = letzteDaten.get(resultat.getObject());
			if ((letztesDatum != null) && (letztesDatum.getData() != null)) {
				sender.sende(resultat);
				letzteDaten.put(resultat.getObject(), resultat);
			}
		} else {
			sender.sende(resultat);
			letzteDaten.put(resultat.getObject(), resultat);
	 ...
	 * <code>qKfz</code>, <code>vLkw</code>, <code>vKfz</code> und <code>vPkw</code>) mit Daten<br>
	 * ggf. (bei DTV-Werten) werden auch die Attribute <code>vLkw</code>, <code>vKfz</code> und
	 * <code>vPkw</code> gefuellt
	 *
	 * @param resultat
	...
	protected final void fuelleRest(final ResultData resultat,
		...
		if (isFahrstreifen()) {
			resultat.getData().getTimeValue("T").setMillis(intervall.getIntervall());
		}
	 ...
	 * @param quellDaten
	 *            die aus dem Puffer ausgelesenen Daten (darf keine leere Liste sein)
	 * @param intervall
	 *            das Aggregationsintervall, fuer dass Daten aus den uebergebenen Quelldaten
	 *            errechnet werden sollen
	 * @param zeitStempel
	 *            der Zeitstempel, fuer den das Datum fuer das uebergebene Intervall aggregiert
	 *            werden soll (gilt eigentlich nur fuer Tagesdaten, da die Anzahl der Stunden pro
	 *            Tag zwischen 23 und 25 schwankt)
	 * @return eine Menge mit so vielen Attribut-Datensaetzen, wie fuer dieses Intervall zur
	 *         Verfuegung stehen muessen
	 */
				...
				for (int i = 0; i < dummy; i++) {
					if (wertAnzahl > 0) {
						if (attribut.isGeschwindigkeitsAttribut()) {
							...
							/**
							 * "halber" Q-Wert muss noch gewichtet werden Das Passiert nur, wenn
							 * z.B. aus KZD, die im 2 Minuten-Takt kommen z.B. das Intervall 5
	...
	/**
	 * Erfragt, ob fuer dieses Objekt ein Datum des uebergebenen Aggregationsintervalls fuer den
	 * uebergebenen Zeitpunkt berechnet werden muss<br>
	 * <b>Achtung:</b> Fuer die Intervalle 1 - 60Minuten ist eine Berechnung immer notwendig. Fuer
	 * alle anderen Intervalle ist eine Berechung nur dann notwendig, wenn nicht schon ein
	 * entsprechendes Datum im Archiv steht (bzw. berechnet wurde)
	 *
	 * @param zeitStempel
	 *            ein Zeitpunkt
	 * @param intervall
	 *            der Aggregationsintervall
	 * @return ob fuer dieses Objekt ein Datum des uebergebenen Aggregationsintervalls fuer den
	 *         uebergebenen Zeitpunkt berechnet werden muss
	 */
	protected final boolean isBerechnungNotwendig(final long zeitStempel,
		...
		if (intervall.equals(AggregationsIntervall.aGG1MINUTE)
			...
			if (!isFahrstreifen()) {
				final AbstraktAggregationsPuffer puffer = datenPuffer.getPuffer(intervall);
				if (puffer != null) {
					berechnen = puffer.getDatenFuerZeitraum(zeitStempel, zeitStempel + 1000)
							.isEmpty();
				}
			}
		}
	...
	protected final void aggregiereMittel(final AggregationsAttribut attribut, final Data nutzDatum,
		 ...
		 * verwendet wurden.
		 */
		final Collection<AggregationsAttributWert> werte = ersetzteAusgefalleneWerte(attribut,
				basisDaten, intervall, zeitStempel);
	 ...
	 *            das Attribut, das berechnet werden soll
	 * @param nutzDatum
	 *            das gesamte Aggregationsdatum (dieses muss veraenderbar sein und wird hier
	 *            gefuellt)
	 * @param basisDaten
	 *            die der Aggregation zu Grunde liegenden Daten
	 * @param zeitStempel
	 ...
	 *            das gewuenschte Aggregationsintervall
	 */
	protected final void aggregiereSumme(final AggregationsAttribut attribut, final Data nutzDatum,
			final Collection<AggregationsDatum> basisDaten, final long zeitStempel,
			final AggregationsIntervall intervall) {
		/**
		 * Die Aggregation erfolgt unabhängig von der Anzahl der gültigen Kurzzeitdatenzyklen.
	 ...
	 */
	public abstract void aggregiere(final long zeitStempel, final AggregationsIntervall intervall);

	/**
	 * Erfragt, ob es sich bei dem hier verwalteten Objekt um eine Objekt vom Typ
